...
docker container run --rm --net test-net node:12.0-alpine sh -c "$wait_for"

echo "Smoke tests..."
docker container run --name tester --rm --net test-net samyual/node-docker sh -c "curl api:3000"
